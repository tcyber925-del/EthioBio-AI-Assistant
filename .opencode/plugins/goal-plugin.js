...
const DEFAULT_OPTIONS = {
  ...
  // Evaluator: independent model-based verification.
  // true = use built-in evaluator with auto-detected alternate model.
  // async function = custom evaluator(input) → { met, reason, evidence }.
  evaluator: true,
  // Model override for the built-in evaluator (maker/checker split).
  // Required to use a DIFFERENT model than the maker (per loop-engineering
  // principles: "the model that wrote the code is too nice grading its own
  // homework"). Format: "provider/model" (e.g. "ollama/tinyllama") or
  // { providerID: string, modelID: string }.
  // When unset, the evaluator auto-detects the session model and picks a
  // different available model. Falls back to same model if none found.
...
function normalizeOptions(options = {}) {
  return {
    ...
    evaluator: options.evaluator === true
      ? true
      : typeof options.evaluator === "function"
        ? options.evaluator
        : DEFAULT_OPTIONS.evaluator,
    evaluatorModel:
      options.evaluatorModel && typeof options.evaluatorModel === "object" && !Array.isArray(options.evaluatorModel)
        ? { providerID: options.evaluatorModel.providerID, modelID: options.evaluatorModel.modelID }
        : typeof options.evaluatorModel === "string" && options.evaluatorModel.includes("/")
          ? (() => {
              const slash = options.evaluatorModel.indexOf("/")
              return { providerID: options.evaluatorModel.slice(0, slash), modelID: options.evaluatorModel.slice(slash + 1) }
            })()
          : undefined,
  }
}
...
// ── Evaluator: independent model-based verification ──────────────
//
// The evaluator is called on session.idle AFTER the assistant finishes a
// turn but BEFORE the auto-continue. It reads the goal condition + recent
// conversation context and returns a structured verdict. This implements
// the maker/checker split from loop engineering: the evaluator is a
// separate model call with no tool access, so it can only judge the
// work the maker produced — it CANNOT grade its own homework.
//
// Per Osmani (Loop Engineering, 2026) and Kopadze (Loops Explained, 2026):
// "The model that wrote the code is far too generous a grader. A second
// agent with different instructions and SOMETIMES a DIFFERENT MODEL catches
// the stuff the first one talked itself into." — Kopadze
// "Your writer can be fast and cheap, your reviewer slow and strict." — Osmani
//
// Configuration:
//   evaluator: true                 → built-in evaluator (auto-discovers alternate model)
//   evaluator: async fn             → custom evaluator
//   evaluator: false                → marker-based (upstream default)
//   evaluatorModel: "p/m" | {...}   → explicit model override

const EVALUATOR_SYSTEM_PROMPT = `\
You are an impartial goal evaluator. Your job is to determine whether a
coding goal has been achieved based only on the conversation evidence
provided. You CANNOT run commands or access files — judge only what is
shown in the transcript.

Rules:
1. The goal is ONLY met if the evidence clearly demonstrates completion.
2. Require explicit verification: test output, command results, file
   contents, or concrete observations — not just the assistant's claims.
3. If the evidence is ambiguous or incomplete, the goal is NOT met.
4. A goal is BLOCKED only if a clear, unrecoverable obstacle is stated
   (missing credentials, unfixable bug, external dependency unavailable).
5. Return your verdict as JSON with these fields:
   - met (boolean): true only if goal is demonstrably achieved
   - blocked (boolean): true only if an unrecoverable blocker exists
   - evidence (string): what verification exists, or "" if none
   - reason (string): explanation of the verdict
`

function buildEvaluatorPrompt(goal, goalCondition, messagesData) {
  const recentMessages = Array.isArray(messagesData) ? messagesData.slice(-6) : []
  ...
  return [
    ...
    `\n</transcript>`,
    `\nRespond with valid JSON only: { "met": boolean, "blocked": boolean, "evidence": string, "reason": string }`,
  ].filter(Boolean).join("\n")
}

function parseEvaluatorResponse(text) {
  if (!text) return null
  // Try to extract JSON from the response
  const jsonMatch = text.match(/\{[\s\S]*"met"\s*:\s*(true|false)[\s\S]*\}/i)
...
async function discoverAvailableModels(client) {
  try {
    ...
    if (!response?.data?.providers) return []
    const models = []
...
async function pickAlternateModel(client, makerModel) {
  if (!makerModel) return null
  const allModels = await discoverAvailableModels(client)
  if (allModels.length === 0) return null

  const makerProvider = makerModel.providerID
  const makerID = makerModel.modelID
  const makerFull = modelID(makerModel)

  // Prefer:
  // 1. Different model from the same provider
  // 2. Any active model that supports text and is different from maker
  // 3. Cheapest model (evaluator only reads + judges, doesn't code)

  let sameProvider = allModels.filter(
    (m) => m.providerID === makerProvider && m.modelID !== makerID && m.capabilities?.input?.text
  )
  if (sameProvider.length > 0) {
    sameProvider.sort((a, b) => (a.cost?.input ?? 0) - (b.cost?.input ?? 0))
    return { providerID: sameProvider[0].providerID, modelID: sameProvider[0].modelID }
  }

  let otherProviders = allModels.filter(
    (m) => modelID(m) !== makerFull && m.capabilities?.input?.text
  )
  if (otherProviders.length > 0) {
    otherProviders.sort((a, b) => (a.cost?.input ?? 0) - (b.cost?.input ?? 0))
    return { providerID: otherProviders[0].providerID, modelID: otherProviders[0].modelID }
  }

  return null
}

function extractMakerModelFromMessages(messagesData) {
  if (!Array.isArray(messagesData)) return null
  for (let i = messagesData.length - 1; i >= 0; i--) {
    const msg = messagesData[i]
    if (msg.info?.role === "assistant" && msg.info?.model) {
...
async function runBuiltinEvaluator(client, sessionID, goal, messagesData) {
  const prompt = buildEvaluatorPrompt(goal, goal.condition, messagesData)
  const body = {
    parts: [makeTextPart(prompt)],
    format: {
      type: "json_schema",
      schema: {
        type: "object",
        properties: {
          met: { type: "boolean", description: "Whether the goal is demonstrably achieved" },
          blocked: { type: "boolean", description: "Whether an unrecoverable blocker exists" },
          evidence: { type: "string", description: "What verification exists, or empty string" },
          reason: { type: "string", description: "Explanation of the verdict" },
        },
        required: ["met", "blocked", "evidence", "reason"],
      },
    },
  }

  // ── Model selection (maker/checker split) ──────────────────────────
  // The evaluator ALWAYS gets an explicit model, never the session default.
  // Priority: explicit evaluatorModel > auto-detected alternate > same model.
  if (goal.options.evaluatorModel) {
    body.model = goal.options.evaluatorModel
  } else {
    const makerModel = extractMakerModelFromMessages(messagesData)
    const alternate = await pickAlternateModel(client, makerModel)
    if (alternate) {
      body.model = alternate
    }
  }
  // Fallback: no model override → same model, but still a separate
  // invocation with no tools and different instructions (maker/checker split).

  let lastError = null
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      const response = await client.session.prompt({ path: { id: sessionID }, body })
      const text = getText(response.data?.parts || [])
      if (!text) continue
  ...
  if (lastError) {
    await logPluginError(client, "Evaluator failed", lastError)
  }
  return null
}

async function runEvaluator(client, sessionID, goal, messagesData) {
  const evaluatorConfig = goal.options.evaluator
  if (!evaluatorConfig) return null
  if (typeof evaluatorConfig === "function") {
    try {
      const result = await evaluatorConfig({ goal, sessionID, messages: messagesData })
      return result && typeof result.met === "boolean"
        ? { met: result.met, blocked: result.blocked === true, evidence: result.evidence || "", reason: result.reason || "" }
        : null
    } catch (error) {
      await logPluginError(client, "Custom evaluator threw", error)
      return null
    }
  }
  return runBuiltinEvaluator(client, sessionID, goal, messagesData)
}
  ...
  const hooks = {
    ...
    event: async ({ event }) => {
      ...
      try {
        ...
        // Runs on every idle event when configured. The evaluator is a
        // separate model call (no tool access) that objectively judges
        // whether the goal condition is met based on transcript evidence.
        // The evaluator verdict takes precedence over marker detection.
        // If the evaluator says met, we complete the goal directly.
        // If the evaluator says blocked, we pause the goal.
        // If the evaluator returns no verdict (or is not configured),
        // fall through to marker-based detection.
        if (activeGoalAfterMessages.options.evaluator) {
          const evaluatorVerdict = await runEvaluator(client, sessionID, activeGoalAfterMessages, messages.data)
          if (evaluatorVerdict) {
            if (evaluatorVerdict.met) {
              const evidence = evaluatorVerdict.evidence || "Completed per independent evaluator."
              await announceAudit(
                sessionID,
                `Evaluator: goal "${summarizeText(activeGoalAfterMessages.condition, 120)}" is satisfied.`,
              )
              activeGoalAfterMessages.lastStatus = `Goal completed (evaluator verdict). Evidence: ${summarizeText(evidence, 200)}`
              pushHistory(
                activeGoalAfterMessages,
                "completed",
                `Independent evaluator confirmed the goal is met: ${summarizeText(evidence, 400)}`,
              )
              rememberGoalResult(sessionID, activeGoalAfterMessages, "achieved", evaluatorVerdict.reason || "", evidence)
              cleanupGoal(sessionID)
              if (sessionOrdered.has(sessionID)) {
                promoteNextOrderedGoal(sessionID)
              }
              await persistTerminalState("completion (evaluator)")
              await announceAudit(
                sessionID,
                `Evaluator result: goal archived as achieved. ${evidence ? `Evidence: ${summarizeText(evidence, 160)}` : ""}`,
              )
              return
            }

            if (evaluatorVerdict.blocked) {
              const reason = evaluatorVerdict.reason || "Blocked per independent evaluator."
              await announceAudit(
                sessionID,
                `Evaluator: goal "${summarizeText(activeGoalAfterMessages.condition, 120)}" is blocked.`,
              )
              activeGoalAfterMessages.blockedReason = reason
              activeGoalAfterMessages.lastStatus = `Goal blocked (evaluator verdict): ${summarizeText(reason, 200)}`
              activeGoalAfterMessages.stopped = true
              activeGoalAfterMessages.stopReason = "blocked"
              pushHistory(activeGoalAfterMessages, "blocked", reason)
              await persistTerminalState("blocked (evaluator)")
              await announceAudit(
                sessionID,
                `Evaluator result: goal paused as blocked — ${summarizeText(reason, 160)}. Run /goal resume after addressing it.`,
              )
              return
            }
          }
