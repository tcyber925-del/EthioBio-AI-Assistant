...
- Task 09: verbatim; both students pages to ErrorState + `normalizeException`, catalog `common.students_load_error` added, audit §6 row updated (`ee63a51`).
- Task 10: all 5 v2 dashboards migrated; widget-level state per plan (Student RecentQuizAttempts, Parent per-child `progressError` — retry only for the selected child, School `schoolError` replaces placeholder). `AIInsightPanel` untouched — pure presentational, no error surfaces. Follow-up tests in `b613d18` (+7 error-path tests; SchoolDashboard dead catch removed).
- Task 11: full ask/voice/workspace migration (`1e09f30`). Deviations: `ask.auth_required` — the state passes an AppError `{category:'authentication', retryable:false}` instead of the string key (redirect to /login retained; key still in catalogs). Ask error title reuses `tc('error')` for non-service errors (`ask.error` was not added; `ask.service_error_title` added en+am). `useConversationHistory`/`ConversationSidebar` left untouched — `error` is a boolean driving a catalog string, no raw text. Voice hooks/buttons `onError` retyped string → AppError; `voice-turn.ts` raw extraction deferred to Task 13 sweep. Quality follow-ups in `f739dd2`: 401s from voice-turn now classified via `fromHttpStatus` (was always service), empty-blob path calls `onError`, `authorization`→`authentication` on ask page, browse dialog close clears `actionError`; deferred: askQuestion/recorder network-throw path (no catch → stuck loading) for the voice-turn sweep.
- Task 12: upload surface is `workspace/upload/page.tsx` (browse/processing contain no upload code — plan's file list corrected). `errors.upload.{unsupported_type,too_large}` en+am; `useErrorMessage` gained `UPLOAD_CODES` tier (`unsupported_type`, `too_large` → `errors.upload.*`, mirrors KNOWN_CODES) + test. Plan's "document or image" copy contradicted the actual `.pdf/.txt/.md` accept — reworded en+am, and the Amharic calque `ይምጡ`→`ያስገቡ` (`48a5b66`). Client size cap aligned to backend pipeline's real `max_file_size_mb=50` (initial 10MB invented limit blocked legitimate textbooks); drag-drop now guarded during upload/success.
- Task 13: the sweep found **~98 RAW_UI sites across 45 files** (plan assumed a handful) — executed in 5 batches + a voice pipeline batch, all reviewed: A1 `05e8be0` (lessons, unit-plans, quiz/take+history incl. [id], recovery, intervention-analytics, knowledge-graph, diagrams), A2 `a498f09` (student, parent, classroom, assignments ×6, quizzes ×2), B1 `075b7b7` (admin page/agents/monitoring/users/schools/review/content ×3/layout — incl. `msg.includes('401'/'403')` sniffing → category check in layout), B2 `46d5c40` (ExecutionPanel, ReflectionTable, GamificationProfile, MisconceptionPanel, ExamReadinessCard, ContinueLearningFeed, ActivityFeed, IconPalette, TTSPlayButton — TTS gained an error surface it never had; wrapper div verified layout-safe in ask page), C `857df0a` (workspace page/browse/[id]/search, dashboard, assessment-studio — loop no longer throws raw `task.error`, breaks on `failed` with `error_failed`/`error_timeout` titles, digital-twin scanned — `{r.detail}` is server data not errors, kept, oauth/callback — only leak was `session_failed` falling into unknown-code branch → now `login.error`), D `67d6587` (voice-turn.ts `RAW_SRC` → structured AppError end-to-end, killing the `f739dd2` regex hack; askQuestion + `recorder.onstop` network-throw escapes caught — the Task 11 deferral; quiz STT `onError={console.error}` → ErrorAlert; TTSPlayButton `audio.onerror` surfaced; +7 voice-turn tests). No catalog keys needed except `monitoring.error_load`. Final search: zero user-facing raw error rendering. Pre-existing gaps documented in audit §6 (admin PATCH unhandled rejections, MisconceptionPanel resolve-error visibility, 403-as-authorized semantics, playTTS silent swallow).
- Task 14: plan's spec code adapted to shipped reality: `fetchWithTimeout` cache-buster (`?_t=`) means the mock glob must be `**/auth/token*`; login input `minLength={6}` blocks a 5-char password; session-expiry specs must plant BOTH `access_token` (middleware) and `auth_ready=1` (page guard, auth.ts) — plan's access_token-only cookies never triggered the mocked refresh; success redirect confirmed `/classroom`. Ran 4/4 PASS against a fresh `next dev -p 3100` (the :3000 instance served a stale pre-plan bundle — always use a fresh port). `5356cb7`.
- Task 15: security audit PASS on all checks (no creds logged/rendered, `cause` dev-gated, detail bodies stripped, stream codes catalog-only, safeNextPath 16-payload tested, next-param single-encode + origin check). Minor findings filed as issues: #103 stale `localStorage('ethiobio_token')` reads in workspace browse download links, #104 oauth callback raw `oauth_error` param render + weak `startsWith('/')` redirect guard, #105 MEDIUM observation — unsanitized `dangerouslySetInnerHTML` markdown render (XSS surface, outside error-scope). Audit doc gained the security checklist. `96a4481`.
- Task 16: `dashboard/docs/error-handling.md` (336 lines) written against shipped code: invariant, AppError shape + 11 categories, classification, resolution chain (KNOWN_CODES → UPLOAD_CODES → SHIPPED_HTTP → categories → generic), adding codes en+am, FieldError mapping, 401/session rules incl. `auth_ready` quirk, component recipes, stream errors, security rules + the Never diagram. `ea60f37`.
- Task 17: full gates PASS (tsc, lint, 137/137 vitest, i18n strict, `npm run build`, e2e 4/4 on fresh port); sweep 1 DERIVED hit only; diff review: 38 commits / 102 files, all in `dashboard/` + `docs/`, backend untouched, no secrets. Final report written to `docs/error-handling-final-report.md`. `999d68c`.
