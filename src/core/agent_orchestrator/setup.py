...
def build_registry(
    ...
    registry.register(AgentRegistration(
        agent=lesson, name="lesson_planner_agent",
        description="Generates structured lesson plans with differentiation and exit tickets",
        capabilities=[AgentCapability(name="lesson_planning", description="Create lesson plans")],
    ))

    diagnostic = DiagnosticAgent(llm_router=router)
    registry.register(AgentRegistration(
        agent=diagnostic, name="diagnostic_agent",
    ...
    registry.register(AgentRegistration(
        agent=translator, name="translator_agent",
    ...
    registry.register(AgentRegistration(
        agent=safety, name="safety_agent",
        description="Reviews content for safety and hallucination",
        capabilities=[AgentCapability(name="safety_review", description="Safety check content")],
    ))

    diagram = DiagramAgent(llm_router=router, adapter=adapter)
    registry.register(AgentRegistration(
        agent=diagram, name="diagram_agent",
        description="Generates biology diagrams",
        capabilities=[AgentCapability(name="diagram_generation", description="Generate diagrams")],
    ))
