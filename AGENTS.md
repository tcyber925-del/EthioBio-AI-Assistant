...
## Ingestion (`scripts/ingest_curriculum.py`)

Re-ingests curriculum PDFs into the vector store with OCR support for garbled PDFs.
...
**EasyOCR** (`--use-easyocr`): Best option for image-heavy PDFs. PyTorch-based, renders at 100 DPI. ~50s/page on CPU. English-only (EasyOCR 1.7 has no Amharic recognition model). Grade 10 re-ingested with this method.
