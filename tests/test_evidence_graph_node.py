...
def test_agentic_graph_node_ordering():
    ...
    assert plan_idx < evidence_idx < sufficient_idx, (
        f"Expected plan_executor < evidence_graph < sufficient_context, got {node_list}"
    )
...
async def test_node_skips_duplicates(mock_evidence_graph_cls):
    ...
    assert mock_graph.add.call_count == 2, f"Expected 2 persists, got {mock_graph.add.call_count}"
    persisted_contents = [call[0][0].content for call in mock_graph.add.call_args_list]
