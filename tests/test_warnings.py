...
def test_build_app_does_not_emit_per_message_warning(monkeypatch):
    ...
    messages = _warning_messages(captured)
    assert not any("per_message" in message for message in messages)
    assert not any("CallbackQueryHandler" in message for message in messages)
    ...
    assert "ConversationHandler" not in handler_types
