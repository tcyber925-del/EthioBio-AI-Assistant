...
async def test_emais_agents_reflection():
    ...
    with patch.object(intervention_agent, '_call_llm', new_callable=AsyncMock) as mock_call:
        mock_call.return_value = {"content": '{"effectiveness_score": 8, "lessons_learned": "Visual aid helped.", "suggested_adjustments": "Use interactive next time."}'}
        res = await intervention_agent.reflect(past_intervention={"type": "visual"}, outcome_data={"score": 85})
    ...
    with patch.object(misconception_agent, '_call_llm', new_callable=AsyncMock) as mock_call:
        mock_call.return_value = {"content": '{"diagnosis_accuracy": 9, "lessons_learned": "Accurate.", "model_updates": "None."}'}
        res = await misconception_agent.reflect(past_diagnosis={"cause": "confuses terms"}, outcome_data={"score": 90})
