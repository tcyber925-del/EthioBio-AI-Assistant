...
export default function AssignmentDetailPage() {
  ...
  if (loading) return <DashboardLayout breadcrumbs={[{ label: t('crumb'), href: '/assignments' }]}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {isTeacher && (
              <div className="bg-v2-surface border border-v2-border rounded-[20px] p-6">
                <h2 className="text-sm font-bold text-v2-text-primary mb-4 flex items-center gap-2">
                  <Users className="w-4 h-4" /> {t('submissions_heading', { count: submissions.length })}
                </h2>
                {submissions.length === 0 ? (
                  <p className="text-sm text-v2-text-secondary">{t('no_submissions')}</p>
                ) : (
                  <div className="flex flex-col gap-3">
                    {submissions.map(s => (
                      <div key={s.id} className="flex items-center justify-between p-4 bg-v2-bg/40 border border-v2-border/40 rounded-xl">
                        <div>
                          <p className="text-sm font-semibold text-v2-text-primary">{t('student_label', { id: s.student_id.slice(0, 8) })}</p>
                          <p className="text-xs text-v2-text-secondary mt-0.5">
                            {t('attempt_label', { n: s.attempt_number })} · {new Date(s.submitted_at).toLocaleString()}
                          </p>
                          {s.status !== 'submitted' && (
                            <p className="text-xs mt-1 font-medium text-v2-success">{t('grade_label', { grade: s.grade ?? '—' })}</p>
                          )}
                        </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-v2-surface border border-v2-border rounded-[20px] p-5 flex flex-col gap-4">
              ...
              <div className="flex items-center gap-3 text-sm text-v2-text-primary">
                <Clock className="w-4 h-4 text-v2-text-secondary" /> {t('max_attempts_label', { count: assignment.max_attempts })}
              </div>
              <div className="flex items-center gap-3 text-sm text-v2-text-primary">
                <ArrowUpCircle className="w-4 h-4 text-v2-text-secondary" />
                {t(assignment.allow_late_submission ? 'late_allowed' : 'no_late')}
              </div>
            </div>
          </div>
        </div>
