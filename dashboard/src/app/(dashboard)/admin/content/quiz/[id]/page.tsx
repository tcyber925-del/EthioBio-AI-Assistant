...
export default function AdminQuizDetailPage() {
  ...
  const load = useCallback(async () => {
    try {
      const response = await fetchWithAuth(`/api/admin/content/quiz/${id}`)
  ...
  if (!quiz) return <p className="text-gray-500">{tcommon('loading')}</p>
  ...
  return (
    <div>
      ...
      <h2 className="text-xl font-semibold mt-8 mb-4">{tc('questions_heading')}</h2>
      {quiz.questions?.map((q: NonNullable<QuizData['questions']>[number], i: number) => {
        ...
        return (
          <div key={q.id} className="border rounded p-4 mb-3">
            ...
            <p className="text-sm text-gray-500 mt-1">{tc('type')} {q.question_type} | {tc('difficulty')} {q.difficulty}</p>
            {opts && opts.length > 0 && (
