    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-foreground-muted text-sm mt-1">Track your child's biology learning progress</p>
        </div>
        {children.length > 1 && (
          <select
            value={selectedId}
      ...
      {children.length === 0 && !loading && (
        <div className="text-center py-16">
          ...
          <p className="text-foreground-muted font-medium">No children linked yet</p>
          <p className="text-xs text-foreground-muted mt-1">Contact your school admin to link your child's account</p>
        </div>
      )}
        ...
        <div className="mb-4 rounded-xl border border-border bg-card p-3 flex items-center gap-4 text-sm">
          ...
          <div>
            ...
            <p className="text-foreground-muted text-xs">
              Grade {selectedChild.grade_level || 'N/A'} &middot; Readiness:{' '}
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className={`rounded-xl border p-4 ${healthBg(progress.overall_readiness)}`}>
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs text-foreground-muted font-medium">Overall Readiness</span>
              ...
              <p className="text-xs text-foreground-muted mt-1">Exam readiness score</p>
            </div>

            <div className="rounded-xl border border-border bg-card p-4">
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs text-foreground-muted font-medium">Current Streak</span>
                <Zap className="w-4 h-4 text-yellow-400" />
              </div>
              <p className="text-2xl font-bold text-foreground">{progress.streak} days</p>
              <p className="text-xs text-foreground-muted mt-1">{progress.total_xp} total XP</p>
            </div>

            <div className="rounded-xl border border-border bg-card p-4">
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs text-foreground-muted font-medium">Topics Mastered</span>
                <Award className="w-4 h-4 text-primary" />
              </div>
              <p className="text-2xl font-bold text-foreground">{Object.keys(progress.mastery_heatmap).length}</p>
              <p className="text-xs text-foreground-muted mt-1">Topics with data</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div className="rounded-xl border border-border bg-card p-4">
              ...
              {Object.keys(progress.mastery_heatmap).length === 0 ? (
                <p className="text-xs text-foreground-muted py-4 text-center">No mastery data yet</p>
              ) : (
            ...
            <div className="rounded-xl border border-border bg-card p-4">
              ...
              {progress.recent_quizzes.length === 0 ? (
                <p className="text-xs text-foreground-muted py-4 text-center">No quizzes taken yet</p>
              ) : (
                <div className="space-y-2">
          ...
          <div className="rounded-xl border border-border bg-card p-4 mb-6">
            <div className="flex items-center justify-between mb-3">
              ...
              >
                {generatingSummary ? 'Generating...' : 'Generate New'}
              </button>
            </div>
            {summary ? (
              <div>
                ...
                {summary.summary_amharic && (
                  <div className="mt-3 pt-3 border-t border-border">
                    <p className="text-xs text-foreground-muted mb-1">አማርኛ</p>
                    <p className="text-sm text-foreground leading-relaxed">{summary.summary_amharic}</p>
                  </div>
                )}
              </div>
            ) : (
              <p className="text-xs text-foreground-muted py-4 text-center">No summary yet. Click "Generate New" to create one.</p>
            )}
          </div>
        </>
      )}
